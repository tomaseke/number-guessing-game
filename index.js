...
function isGuessInRange(guess, range) {
  if (guess < 0 || guess > range) {
    probability.innerHTML =
      "Your guess isn't in your range. Select a different number dumbass.";
    result.innerHTML = "";
...
function calculateProbability(range) {
  let probability = (1 / (range + 1)) * 100;
  if (probability > 0 && Number.isInteger(probability)) {
    return `The probability that you will guess the number is ${probability}%.`;
  }

  if (probability > 1 && !Number.isInteger(probability)) {
    return `The probability that you will guess the number is ${(
      (1 / (range + 1)) *
      100
    ).toFixed(2)}%.`;
  }
